...
class GameManager:
    ...
    @property
    def current_seller(self) -> list[Seller]:
        return self.seller[self.current_map_key]
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        for key, m in self.maps.items():
            ...
            block["seller"] = [s.to_dict() for s in self.seller.get(key, [])]
    ...
    def from_dict(cls, data: dict[str, object]) -> "GameManager":
        ...
        gm = cls(maps, current_map, None, trainers, bag=None)  # Player
        ...
        for m in data["map"]:
            raw_sellers = m["seller"]
            gm.seller[m["path"]] = [Seller.from_dict(s, gm) for s in raw_sellers]
    ...
    def in_happy(self):
        return self.current_map.path_name == "happyhappy.tmx"
    ...
    def atk_cal(self, base, level):
        return int( ((base * level / 100)**0.3) + 0.25 * level + 5)

    def hp_cal(self, base, level):
        return int((base * (level**0.8) / 200) + 0.4 * level + 10)
    
    def update_gscene(self,info):
        self.gscene=True
        self.gsceneinfo=info

    def call_gscene(self):
        return self.gscene
    def end_call_gscene(self):
        self.gscene=False
        return self.gsceneinfo